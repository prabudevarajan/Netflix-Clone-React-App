...
function Row({ title, fetchUrl, isLargeRow }) {
  ...
  const clickopt = (movie) => {
    if (trailerUrl) {
      ...
      fetch(
        `https://youtube.googleapis.com/youtube/v3/search?maxResults=6&q=movie trailer${
          movie?.title || movies?.name || movies?.original_name || ""
        }&key=AIzaSyBxsZcBe2MJOQO3lllZuqyesjbdajJesJo`
      )
